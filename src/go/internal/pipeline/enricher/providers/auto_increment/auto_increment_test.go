...
func TestAutoIncrement_Enrich(t *testing.T) {
	...
	t.Run("Skips if title filter mismatch", func(t *testing.T) {
		...
		activity := &pbactivity.StandardizedActivity{Name: "Afternoon Walk"}
		user := &user.Record{UserProfile: &pbuser.UserProfile{UserId: "u1"}}
	...
	t.Run("Increments existing counter", func(t *testing.T) {
		...
		mockDB := &mocks.MockDatabase{
			GetCounterFunc: func(ctx context.Context, userId, id string) (*pbuser.Counter, error) {
				return &pbuser.Counter{Id: "parkrun", Count: 5}, nil
			},
			SetCounterFunc: func(ctx context.Context, userId string, counter *pbuser.Counter) error {
				setCounter = counter
				return nil
	...
	t.Run("Matches title case insensitive", func(t *testing.T) {
		...
		mockDB := &mocks.MockDatabase{
			GetCounterFunc: func(ctx context.Context, userId, id string) (*pbuser.Counter, error) {
				return &pbuser.Counter{Id: "parkrun", Count: 0}, nil
			},
			SetCounterFunc: func(ctx context.Context, userId string, counter *pbuser.Counter) error {
				setCounter = counter
				return nil
...
func TestAutoIncrement_CounterRules(t *testing.T) {
	...
	t.Run("No rule matches skips increment", func(t *testing.T) {
		...
		activity := &pbactivity.StandardizedActivity{Name: "Weight Training"}
		user := &user.Record{UserProfile: &pbuser.UserProfile{UserId: "u1"}}
		inputs := map[string]string{
			"counter_rules": `{"parkrun": "parkrun_counter"}`,
		}
	...
	t.Run("Case-insensitive counter_rules matching", func(t *testing.T) {
		...
		mockDB := &mocks.MockDatabase{
			GetCounterFunc: func(ctx context.Context, userId, id string) (*pbuser.Counter, error) {
				return &pbuser.Counter{Id: "leg_day", Count: 3}, nil
			},
			SetCounterFunc: func(ctx context.Context, userId string, counter *pbuser.Counter) error {
				setCounter = counter
				return nil
		...
		activity := &pbactivity.StandardizedActivity{Name: "LEG DAY Session"}
		user := &user.Record{UserProfile: &pbuser.UserProfile{UserId: "u1"}}
		inputs := map[string]string{
