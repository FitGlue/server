...
func TestLogicGate_AllMatchContinue(t *testing.T) {
	...
	act := &pbactivity.StandardizedActivity{Type: pbactivity.ActivityType_ACTIVITY_TYPE_RUN, StartTime: timestamppb.New(time.Now())}
...
func TestLogicGate_NegatedTimeHalt(t *testing.T) {
	...
	act := &pbactivity.StandardizedActivity{StartTime: timestamppb.New(start)}
	res, err := NewLogicGateProvider().Enrich(context.Background(), slog.Default(), act, nil, inputs, false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestLogicGate_AnyMatchContinue(t *testing.T) {
	...
	cfgBytes, _ := json.Marshal(cfg)
	inputs := map[string]string{"logic_config": string(cfgBytes)}
	act := &pbactivity.StandardizedActivity{Name: "Morning Run", Type: pbactivity.ActivityType_ACTIVITY_TYPE_RUN, StartTime: timestamppb.New(time.Now())}
	res, err := NewLogicGateProvider().Enrich(context.Background(), slog.Default(), act, nil, inputs, false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
