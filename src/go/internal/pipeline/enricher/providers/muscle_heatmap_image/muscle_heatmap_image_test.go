...
func TestMuscleHeatmapImageProvider_ProviderType(t *testing.T) {
	provider := NewMuscleHeatmapImageProvider()
	if provider.ProviderType() != pbplugin.EnricherProviderType_ENRICHER_PROVIDER_MUSCLE_HEATMAP_IMAGE {
	...
	sets := []*pbactivity.StrengthSet{
		{
			...
			PrimaryMuscleGroup: pbactivity.MuscleGroup_MUSCLE_GROUP_CHEST,
			WeightKg:           100,
			Reps:               10,
		},
		{
			ExerciseName:       "Squat",
			PrimaryMuscleGroup: pbactivity.MuscleGroup_MUSCLE_GROUP_QUADRICEPS,
			WeightKg:           150,
			Reps:               8,
		},
	}
