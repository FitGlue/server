...
func TestIntervals_NoIntervalData(t *testing.T) {
	...
	result, err := p.Enrich(context.Background(), slog.Default(), activity, &user.Record{}, nil, false)
	if err != nil {
...
func TestIntervals_BasicIntervals(t *testing.T) {
	...
	activity := &pbactivity.StandardizedActivity{
		...
		Workout: &pbactivity.WorkoutDefinition{
			Name: "Test Sprints",
		},
	}
...
func TestIntervals_ShowAllIntervals(t *testing.T) {
	...
	result, err := p.Enrich(context.Background(), slog.Default(), activity, &user.Record{}, inputs, false)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
