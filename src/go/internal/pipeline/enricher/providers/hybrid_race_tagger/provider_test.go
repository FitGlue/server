...
func makeLap(distance, duration float64, startOffset time.Duration) *pbactivity.Lap {
	baseTime := time.Date(2024, 1, 1, 10, 0, 0, 0, time.UTC)
	return &pbactivity.Lap{
...
func makeLapWithRecords(distance, duration float64, startOffset time.Duration, recordCount int) *pbactivity.Lap {
	lap := makeLap(distance, duration, startOffset)
	for i := 0; i < recordCount; i++ {
		lap.Records = append(lap.Records, &pbactivity.Record{})
	}
	return lap
}
