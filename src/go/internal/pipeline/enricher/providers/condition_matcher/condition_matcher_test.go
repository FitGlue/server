...
func TestConditionMatcher_Enrich(t *testing.T) {
	...
	t.Run("Matches Type", func(t *testing.T) {
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Mon", 10), Type: pbactivity.ActivityType_ACTIVITY_TYPE_RUN}
	...
	t.Run("Fails Type Mismatch", func(t *testing.T) {
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Mon", 10), Type: pbactivity.ActivityType_ACTIVITY_TYPE_SWIM}
		inputs := map[string]string{
			"activity_type": "run",
		}
		res, _ := provider.Enrich(ctx, slog.Default(), act, nil, inputs, false)
	...
	t.Run("Matches Day", func(t *testing.T) {
		// Sat 10am
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Sat", 10)}
		inputs := map[string]string{
			"days":           "Sat,Sun",
	...
	t.Run("Fails Day Mismatch", func(t *testing.T) {
		// Mon 10am
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Mon", 10)}
		inputs := map[string]string{
			"days": "Sat,Sun",
		}
		res, _ := provider.Enrich(ctx, slog.Default(), act, nil, inputs, false)
	...
	t.Run("Matches Time Range", func(t *testing.T) {
		// 9am
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Sat", 9)}
		inputs := map[string]string{
			"time_start":     "08:30",
			"time_end":       "09:30",
	...
	t.Run("Matches Numeric Day", func(t *testing.T) {
		// Sunday
		act := &pbactivity.StandardizedActivity{StartTime: makeTime("Sun", 10)}
		inputs := map[string]string{
