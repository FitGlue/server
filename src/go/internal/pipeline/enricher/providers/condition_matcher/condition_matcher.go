...
func (p *ConditionMatcherProvider) ProviderType() pbplugin.EnricherProviderType {
	return pbplugin.EnricherProviderType_ENRICHER_PROVIDER_CONDITION_MATCHER
}

func (p *ConditionMatcherProvider) Enrich(ctx context.Context, logger *slog.Logger, act *pbactivity.StandardizedActivity, user *user.Record, inputs map[string]string, doNotRetry bool) (*providers.EnrichmentResult, error) {
	...
	if val, ok := inputs["activity_type"]; ok && val != "" {
		...
		if expectedType != pbactivity.ActivityType_ACTIVITY_TYPE_UNSPECIFIED && act.Type != expectedType {
