...
func reconcileTimeMarkerLabels(activity *pbactivity.StandardizedActivity) {
	...
	// Skip if these markers were explicitly generated by the hybrid race tagger
	// which already assigns perfect labels to both runs and stations.
	if activity.TimeMarkers[0].MarkerType == "run_start" || activity.TimeMarkers[0].MarkerType == "station_start" {
		return
	}
