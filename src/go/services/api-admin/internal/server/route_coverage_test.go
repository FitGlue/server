...
// tagged as AdminGatewayService has a corresponding Chi route registered.
func TestRouteCoverage(t *testing.T) {
	specPath := "../../../../../../docs/api/gateway/admin.openapi.yaml"
	...
	// Extract all AdminGatewayService paths from the spec
	...
	for path, methods := range spec.Paths {
		for method, detail := range methods {
			for _, tag := range detail.Tags {
				if tag == "AdminGatewayService" {
					expectedRoutes = append(expectedRoutes, specRoute{
						path:   "/api/admin" + path, // Routes are registered under /api/admin
	...
	if len(expectedRoutes) == 0 {
		t.Fatal("No AdminGatewayService routes found in OpenAPI spec — is the spec generated correctly?")
	}

	t.Logf("Found %d AdminGatewayService routes in OpenAPI spec", len(expectedRoutes))

	// Build the actual router using NewAPIServer with mock service clients.
	srv := NewAPIServer(
		infra.NewLogger(),
		nil, // authClient — only need router structure
		&adminMockUserClient{},
