...
func init() {
	enricher_providers.Register(NewLocationNaming())
}
...
func (p *LocationNaming) Enrich(ctx context.Context, activity *pb.StandardizedActivity, user *pb.UserRecord, inputs map[string]string, doNotRetry bool) (*enricher_providers.EnrichmentResult, error) {
	...
	if cached {
		...
		if err != nil {
			...
			return nil, &enricher_providers.RetryableError{Err: fmt.Errorf("failed to create request: %w", err)}
		}
		...
		if err != nil {
			...
			return nil, &enricher_providers.RetryableError{Err: fmt.Errorf("nominatim API request failed: %w", err)}
		}
		...
		if resp.StatusCode != http.StatusOK {
			...
			return nil, &enricher_providers.RetryableError{Err: fmt.Errorf("nominatim API returned status %d", resp.StatusCode)}
		}
