...
func handleTrainingPeaksCreate(ctx context.Context, httpClient *http.Client, eventPayload *pb.EnrichedActivityEvent, athleteID string, fwCtx *framework.FrameworkContext) (interface{}, error) {
	...
	// Key is destination:destinationId so when TrainingPeaks sends a webhook,
	// we can look it up and detect the bounceback
	if workoutID != "" {
		uploadRecord := &pb.UploadedActivityRecord{
			Id:            loopprevention.BuildUploadedActivityID(pb.Destination_DESTINATION_TRAININGPEAKS, workoutID),
	...
	destination.UpdateStatus(ctx, svc.DB, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_TRAININGPEAKS, pb.DestinationStatus_DESTINATION_STATUS_SUCCESS, workoutID, "", fwCtx.Logger)
