...
func uploadHandler(httpClient *http.Client) framework.HandlerFunc {
	return func(ctx context.Context, e event.Event, fwCtx *framework.FrameworkContext) (interface{}, error) {
		...
		if err != nil {
			destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("failed to get user: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
			return nil, fmt.Errorf("failed to get user: %w", err)
		}
		if user.Integrations == nil || user.Integrations.Intervals == nil || !user.Integrations.Intervals.Enabled {
			destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", "Intervals integration not enabled", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		...
		if intervalsIntegration.ApiKey == "" || intervalsIntegration.AthleteId == "" {
			destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", "missing API key or athlete ID", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
...
func handleIntervalsCreate(ctx context.Context, httpClient *http.Client, integration *pb.IntervalsIntegration, eventPayload *pb.EnrichedActivityEvent, fwCtx *framework.FrameworkContext) (interface{}, error) {
	...
	if err != nil {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("GCS error: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if err != nil {
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("failed to create request: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if err != nil {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("API error: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if resp.StatusCode >= 400 {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("API error (HTTP %d): %s", resp.StatusCode, err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return nil, err
	}

	var uploadResp intervalsActivityResponse
	if err := json.NewDecoder(resp.Body).Decode(&uploadResp); err != nil {
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("failed to decode response: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_SUCCESS, intervalsDestID, "", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
...
func handleIntervalsUpdate(ctx context.Context, httpClient *http.Client, integration *pb.IntervalsIntegration, eventPayload *pb.EnrichedActivityEvent, fwCtx *framework.FrameworkContext) (interface{}, error) {
	...
	if len(updateBody) == 0 {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_INTERVALS, pb.DestinationStatus_DESTINATION_STATUS_SUCCESS, intervalsIDStr, "", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return map[string]interface{}{
			"status":                "SUCCESS",
