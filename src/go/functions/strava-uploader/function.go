...
func handleStravaCreate(ctx context.Context, httpClient *http.Client, eventPayload *pb.EnrichedActivityEvent, fwCtx *framework.FrameworkContext) (interface{}, error) {
	...
	if err != nil {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("GCS error: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if err != nil {
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("failed to create request: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if err != nil {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("API error: %s", err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if httpResp.StatusCode >= 400 {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", fmt.Sprintf("HTTP %d: %s", httpResp.StatusCode, err), eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return nil, err
	}
	...
	if derefStr(uploadResp.Error) != "" && strings.Contains(strings.ToLower(derefStr(uploadResp.Error)), "duplicate") {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_SKIPPED, "", "duplicate_activity", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
	...
	if status != "SUCCESS" {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_FAILED, "", errMsg, eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return result, fmt.Errorf("strava upload incomplete: %s", errMsg)
	}

	// Update PipelineRun destination as synced
	stravaDestID := fmt.Sprintf("%d", derefInt64(uploadResp.ActivityId))
	destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_SUCCESS, stravaDestID, "", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
...
func handleStravaUpdate(ctx context.Context, httpClient *http.Client, eventPayload *pb.EnrichedActivityEvent, fwCtx *framework.FrameworkContext) (interface{}, error) {
	...
	if err != nil || pipelineRun == nil {
		...
		if stravaIDStr == "" {
			...
			destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_SKIPPED, "", "activity_not_found", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
			return map[string]interface{}{
				"status":      "SKIPPED",
	...
	if stravaIDStr == "" {
		...
		// Update PipelineRun destination as skipped
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_SKIPPED, "", "no_strava_destination", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return map[string]interface{}{
			"status":      "SKIPPED",
	...
	if len(updateBody) == 0 {
		...
		destination.UpdateStatus(ctx, svc.DB, svc.Notifications, eventPayload.UserId, fwCtx.PipelineExecutionId, pb.Destination_DESTINATION_STRAVA, pb.DestinationStatus_DESTINATION_STATUS_SUCCESS, stravaIDStr, "", eventPayload.Name, eventPayload.ActivityId, fwCtx.Logger)
		return map[string]interface{}{
			"status":             "SUCCESS",
