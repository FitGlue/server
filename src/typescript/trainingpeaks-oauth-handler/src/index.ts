...
const handler = async (req: any, res: any, ctx: FrameworkContext) => {
  ...
  if (error) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/trainingpeaks/error?reason=denied`);
    return;
  }
  ...
  if (!code || !state) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/trainingpeaks/error?reason=missing_params`);
    return;
  }
  ...
  if (!validation.valid || !validation.userId) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/trainingpeaks/error?reason=invalid_state`);
    return;
  }
  ...
  try {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/trainingpeaks/success`);

  } catch (error: unknown) {
    logger.error('Error processing TrainingPeaks OAuth callback', { error });
    res.redirect(`${process.env.BASE_URL}/app/connections/trainingpeaks/error?reason=server_error`);
  }
};
