...
const handler = async (req: any, res: any, ctx: FrameworkContext) => {
  ...
  if (error) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/wahoo/error?reason=denied`);
    return;
  }
  ...
  if (!code || !state) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/wahoo/error?reason=missing_params`);
    return;
  }
  ...
  if (!validation.valid || !validation.userId) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/wahoo/error?reason=invalid_state`);
    return;
  }
  ...
  try {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/wahoo/success`);

  } catch (error: unknown) {
    logger.error('Error processing Wahoo OAuth callback', { error });
    res.redirect(`${process.env.BASE_URL}/app/connections/wahoo/error?reason=server_error`);
  }
};
