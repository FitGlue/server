...
import { wahooOAuthHandler } from './index';

describe('wahooOAuthHandler', () => {
  let req: any;
  let ctx: any;
  ...
  it('should redirect to error page if user denies authorization', async () => {
    req.query = { error: 'access_denied', error_description: 'User denied' };

    const result = await (wahooOAuthHandler as any)(req, ctx);

    expect(ctx.logger.warn).toHaveBeenCalledWith('User denied Wahoo authorization', { error: 'access_denied', errorDescription: 'User denied' });
    expect(result).toEqual({
      statusCode: 302,
      headers: { Location: 'https://dev.fitglue.tech/app/connections/wahoo/error?reason=denied' }
    });
  });
  ...
  it('should redirect to error page if code is missing', async () => {
    ...
    expect(result).toEqual({
      statusCode: 302,
      headers: { Location: 'https://dev.fitglue.tech/app/connections/wahoo/error?reason=missing_params' }
    });
  });
  ...
  it('should redirect to error page if state token is invalid', async () => {
    ...
    expect(result).toEqual({
      statusCode: 302,
      headers: { Location: 'https://dev.fitglue.tech/app/connections/wahoo/error?reason=invalid_state' }
    });
  });
  ...
  it('should successfully process OAuth callback and store tokens', async () => {
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith('Successfully connected Wahoo account', {
      userId: 'user-123',
      wahooUserId: 12345,
    });
    expect(result).toEqual({
      statusCode: 302,
      headers: { Location: 'https://dev.fitglue.tech/app/connections/wahoo/success' }
    });
  });
  ...
  it('should redirect to error page if token exchange fails', async () => {
    ...
    expect(ctx.logger.error).toHaveBeenCalledWith('Error processing Wahoo OAuth callback', expect.anything());
    expect(result).toEqual({
      statusCode: 302,
      headers: { Location: 'https://dev.fitglue.tech/app/connections/wahoo/error?reason=server_error' }
    });
  });
});
