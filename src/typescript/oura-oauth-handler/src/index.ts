...
const handler = async (req: any, res: any, ctx: FrameworkContext) => {
  ...
  if (error) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/oura/error?reason=denied`);
    return;
  }
  ...
  if (!code || !state) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/oura/error?reason=missing_params`);
    return;
  }
  ...
  if (!validation.valid || !validation.userId) {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/oura/error?reason=invalid_state`);
    return;
  }
  ...
  try {
    ...
    res.redirect(`${process.env.BASE_URL}/app/connections/oura/success`);

  } catch (error: unknown) {
    logger.error('Error processing Oura OAuth callback', { error });
    res.redirect(`${process.env.BASE_URL}/app/connections/oura/error?reason=server_error`);
  }
};
