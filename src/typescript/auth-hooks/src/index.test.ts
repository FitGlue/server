...
describe('Auth Hooks: authOnCreate', () => {
  ...
  let authOnCreate: (event: CloudEvent<any>) => Promise<void>;
  ...
  it('should call userService.createUser with the UID from the event subject', async () => {
    const mockEvent = new CloudEvent({
      type: 'google.firebase.auth.user.v1.created',
      source: '//firebaseauth.googleapis.com/projects/my-project',
      subject: 'users/test-uid-123',
      data: {
        uid: 'test-uid-123',
    ...
    await authOnCreate(mockEvent as any);

    expect(mockCreateUser).toHaveBeenCalledWith('test-uid-123');
  });

  it('should handle events without a valid subject gracefully', async () => {
    const mockEvent = new CloudEvent({
      type: 'google.firebase.auth.user.v1.created',
      source: '//firebaseauth.googleapis.com/projects/my-project',
      // subject omitted
